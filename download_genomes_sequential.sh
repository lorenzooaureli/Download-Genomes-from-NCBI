...
# =============================================================================
# NCBI Genome Downloader (Sequential Version)
# =============================================================================
#
# This script downloads genome sequences from NCBI based on accession numbers.
# Unlike the parallel version, this script downloads genomes sequentially.
#
# Usage:
#   1. Download genomes from a file containing accession numbers:
#      ./download_genomes_sequential.sh [--compress] [--h_version] accessions.txt output_dir
#
#   2. Download genomes by specifying accessions directly:
#      ./download_genomes_sequential.sh [--compress] [--h_version] --accession GCA_000001405.29 GCA_000002035.4 output_dir
#
# Parameters:
...
#
# Examples:
#   ./download_genomes_sequential.sh genomes.txt ./downloaded_genomes
#   ./download_genomes_sequential.sh --compress genomes.txt ./downloaded_genomes
#   ./download_genomes_sequential.sh --h_version --accession GCA_000013925.1 ./my_genomes
#   ./download_genomes_sequential.sh --compress --h_version --accession GCA_000001405.29 GCA_000002035.4 ./my_genomes
#
# =============================================================================
...
if [ ${#ACCESSIONS[@]} -eq 0 ] && [ $# -lt 2 ]; then
  echo "Usage: $0 [--compress] [--h_version] [--accession GCA_XXXXXX GCA_YYYYYY ...] <output_directory>"
  echo "   OR: $0 [--compress] [--h_version] <accession_file> <output_directory>"
  exit 1
fi
...
# Process each accession sequentially
if [ "$HIGHEST_VERSION" = "true" ]; then
  echo "Using highest version mode - will search for the latest version of each accession"
fi

# Read accessions from file and process them one by one
while IFS= read -r accession; do
  # Skip empty lines or lines that don't match the accession pattern
  if [[ -z "$accession" || ! "$accession" =~ ^GC[AF]_[0-9]+(\.[0-9]+)?$ ]]; then
    echo "Skipping invalid accession: $accession"
    continue
  fi
  
  download_genome_ftp "$accession" "$OUTPUT_DIR" "$COMPRESS" "$HIGHEST_VERSION"
done < "$ACCESSION_FILE"
